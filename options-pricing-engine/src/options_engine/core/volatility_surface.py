...
class VolatilitySurface:
    ...
    def update_volatility(
        ...
        *,
        ...
        with self._lock:
            ...
            self._cache.clear()
            self._interpolator = self._build_interpolator_locked()

    def _build_interpolator_locked(
        self,
    ) -> Optional[Callable[[Sequence[Sequence[float]]], np.ndarray]]:
    ...
    def get_volatility(self, strike: float, maturity: float, spot: float) -> float:
        now = time.time()
        key = (round(strike, 4), round(maturity, 4), round(spot, 4))
        ...
        with self._lock:
            ...
            if not self._points:
                volatility = 0.20
            elif self._interpolator is None or len(self._points) < 4:
