...
The exact deterministic cash-dividend model, support matrix, event ordering,
and reference evidence are in [Discrete dividends](docs/DISCRETE_DIVIDENDS.md).
...
market = MarketData(
    ...
    risk_free_rate=0.04,
    dividend_yield=0.005,
    cash_dividends=CashDividendSchedule((CashDividend(ex_time=0.25, amount=1.25),)),
)
result = BinomialModel(steps=800).calculate_price(contract, market, volatility=0.24)
```
