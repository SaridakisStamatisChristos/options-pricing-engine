...
def aggregate_statistics(contributions: np.ndarray) -> GreekSummary:
    ...
    if sample.ndim != 1:
        raise ValueError("contributions must be one-dimensional")
    if sample.size > 1_000_000:
        raise ValueError("contributions exceeds the 1000000-value limit")
    if sample.size == 0:
        return GreekSummary(
...
def _indicator(contract: OptionContract, terminal_prices: np.ndarray) -> np.ndarray:
    if contract.option_type is OptionType.CALL:
        return np.asarray(np.greater(terminal_prices, contract.strike_price), dtype=bool)
    return np.asarray(np.less(terminal_prices, contract.strike_price), dtype=bool)
...
def pathwise_gamma(
    ...
    *,
    ...
    score_s0 = (
        log_ratio - (market_data.risk_free_rate - market_data.dividend_yield + 0.5 * sigma_sq) * tau
...
def theta_likelihood_ratio(
    ...
    *,
    ...
    drift = market_data.risk_free_rate - market_data.dividend_yield - 0.5 * sigma_sq
    deviation = log_ratio - drift * tau
    score_tau = -0.5 / tau + (deviation**2 + 2.0 * deviation * drift * tau) / (
        2.0 * sigma_sq * tau**2
    )
    ...
    # Market theta is the derivative with respect to calendar time, the
    # negative of the derivative with respect to remaining maturity.
    return np.asarray(-contributions / 365.0, dtype=float)
...
def rho_likelihood_ratio(
    ...
    *,
    ...
    return np.asarray(contributions / 100.0, dtype=float)
...
def simulate_terminal_prices(
    ...
    if any(
        isinstance(value, bool) or not isinstance(value, Real) for value in scalar_inputs.values()
    ):
        raise TypeError("simulation scalar inputs must be real numbers")
    if not all(math.isfinite(float(value)) for value in scalar_inputs.values()):
    ...
    if not 0.0 <= volatility <= 5.0:
        raise ValueError("volatility must be within [0, 5]")
    if not 0.0 <= time_to_expiry <= 100.0:
        raise ValueError("time_to_expiry must be within [0, 100]")
    draws_array = np.asarray(draws, dtype=float)
    if draws_array.ndim != 1:
        raise ValueError("draws must be one-dimensional")
    if draws_array.size > 1_000_000:
        raise ValueError("draws exceeds the 1000000-value limit")
    if not np.isfinite(draws_array).all():
        raise ValueError("draws must be finite")

    tau = float(time_to_expiry)
    if tau == 0.0:
        return np.full_like(draws_array, fill_value=spot, dtype=float)
    sigma = float(volatility)
    sqrt_tau = math.sqrt(tau)
    drift = (market_data.risk_free_rate - market_data.dividend_yield - 0.5 * sigma**2) * tau
    diffusion = sigma * sqrt_tau * draws_array
    with np.errstate(over="ignore", invalid="ignore"):
        terminal = np.asarray(float(spot) * np.exp(drift + diffusion), dtype=float)
    if not np.isfinite(terminal).all():
        raise ValueError("simulation exceeds the supported floating-point range")
    return terminal
...
def finite_difference_delta(
    ...
    *,
    ...
    down_terminal = simulate_terminal_prices(
        down_spot, market_data, volatility, time_to_expiry, draws
    )
...
def finite_difference_vega(
    ...
    *,
    ...
    up_sigma = min(sigma + bump, 5.0)
    down_sigma = max(sigma - bump, SIGMA_FLOOR)
    discount_factor = math.exp(-market_data.risk_free_rate * time_to_expiry)
    up_terminal = simulate_terminal_prices(
        market_data.spot_price, market_data, up_sigma, time_to_expiry, draws
    )
    down_terminal = simulate_terminal_prices(
        market_data.spot_price, market_data, down_sigma, time_to_expiry, draws
    )
...
def finite_difference_theta(
    ...
    *,
    ...
    up_tau = min(tau + bump, 100.0)
    down_tau = max(tau - bump, 0.0)
    up_discount = math.exp(-market_data.risk_free_rate * up_tau)
    down_discount = math.exp(-market_data.risk_free_rate * down_tau)
    up_terminal = simulate_terminal_prices(
        market_data.spot_price, market_data, volatility, up_tau, draws
    )
    down_terminal = simulate_terminal_prices(
        market_data.spot_price, market_data, volatility, down_tau, draws
    )
...
def finite_difference_rho(
    ...
    *,
    ...
    bump = 1e-6
    up_rate = min(rate + bump, 1.0)
    down_rate = max(rate - bump, -1.0)
    ...
    up_terminal = simulate_terminal_prices(
        market_data.spot_price, up_market, volatility, time_to_expiry, draws
    )
    down_terminal = simulate_terminal_prices(
        market_data.spot_price, down_market, volatility, time_to_expiry, draws
    )
    up_disc = up_discount * _payoff(contract, up_terminal)
    down_disc = down_discount * _payoff(contract, down_terminal)
    h_up = up_rate - rate
...
def ensure_fd_inputs(
    ...
    *,
    ...
    if draws.ndim != 1 or discounted_payoffs.ndim != 1:
        raise ValueError("draws and discounted payoffs must be one-dimensional")
    if draws.shape != discounted_payoffs.shape:
        raise ValueError("draws and discounted payoffs must have matching shapes")
    if draws.size == 0 or draws.size > 1_000_000:
        raise ValueError("finite-difference sample size must be within [1, 1000000]")
    if not guard_against_pathologies([draws, discounted_payoffs]):
