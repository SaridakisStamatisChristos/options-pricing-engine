...
def clamp_log_moneyness(values: np.ndarray) -> np.ndarray:
    ...
    return np.asarray(np.clip(values, -LOG_MONEYNESS_BOUND, LOG_MONEYNESS_BOUND), dtype=float)
...
def is_estimate_unstable(
    value: float, half_width_abs: float, price: float, *, threshold: float = 0.2
) -> bool:
