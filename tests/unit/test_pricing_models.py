...
def _finite_difference_greeks(
    ...
    vol_down = model.calculate_price(
        contract, market, max(volatility - vol_bump, 1e-8)
    ).theoretical_price
...
def test_model_constructors_reject_invalid_and_unsafe_workloads() -> None:
    with pytest.raises(TypeError, match="steps must be an integer"):
        BinomialModel(steps=2.5)
    with pytest.raises(ValueError, match="steps"):
        BinomialModel(steps=1)
    with pytest.raises(ValueError, match="paths"):
        MonteCarloModel(paths=0)
    with pytest.raises(TypeError, match="antithetic"):
        MonteCarloModel(paths=100, antithetic=1)
    with pytest.raises(ValueError, match=r"steps\*paths"):
        LongstaffSchwartzModel(paths=10_000, steps=501)
    with pytest.raises(ValueError, match="reference_steps"):
        LongstaffSchwartzModel(reference_steps=5_000)


def test_lsmc_helper_enforces_resource_limits_before_allocation() -> None:
    with pytest.raises(ValueError, match="steps"):
        american_lsmc_price(
            100.0,
            100.0,
...
def test_binomial_rejects_unrepresentable_terminal_tree() -> None:
    model = BinomialModel(steps=4_096)
    contract = OptionContract(
        symbol="RANGE",
        strike_price=1e12,
    ...
    market = MarketData(spot_price=1e12, risk_free_rate=0.0)

    with pytest.raises(ValueError, match="floating-point range"):
        model.calculate_price(contract, market, 5.0)
    ...
    for maturity in maturities:
        for volatility in volatilities:
            ...
            for strike in strikes:
                ...
                call_price = model.calculate_price(
                    call_contract, market, volatility
                ).theoretical_price
                put_price = model.calculate_price(
                    put_contract, market, volatility
                ).theoretical_price

                parity = market.spot_price * math.exp(
                    -market.dividend_yield * maturity
                ) - strike * math.exp(-market.risk_free_rate * maturity)
...
def test_call_prices_are_convex_in_strike() -> None:
    ...
    for maturity in maturities:
        for volatility in volatilities:
            ...
            for left, center, right in zip(prices, prices[1:-1], prices[2:], strict=False):
...
def test_call_price_monotone_in_maturity_and_volatility() -> None:
    ...
    for strike in strikes:
        contracts = [
            OptionContract(
                symbol="CALL", strike_price=strike, time_to_expiry=m, option_type=OptionType.CALL
            )
            for m in maturities
        ]
        ...
        base_vol = 0.2
        maturity_prices = [
            model.calculate_price(contract, market, base_vol).theoretical_price
            for contract in contracts
        ]

        for earlier, later in itertools.pairwise(maturity_prices):
        ...
        vol_prices = [
            model.calculate_price(contract, market, vol).theoretical_price for vol in volatilities
        ]

        for lower, higher in itertools.pairwise(vol_prices):
...
def test_lsmc_is_statistically_consistent_with_high_res_binomial() -> None:
    ...
    standard_error = analysis.pricing_result.standard_error
    assert standard_error is not None
    statistical_tolerance_bps = 3.0 * standard_error / analysis.reference_price * 10_000.0
    assert abs(analysis.price_diff_bps) <= max(5.0, statistical_tolerance_bps)
...
def test_lsmc_provides_basis_selection_and_policy_details() -> None:
    ...
    selected_bases = [
        step.basis
        for step in analysis.policy_steps
        if step.basis is not None and step.in_the_money > 0
    ]
