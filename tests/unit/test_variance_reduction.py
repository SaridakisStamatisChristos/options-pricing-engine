...
def test_qmc_strategy_reproducible_with_common_random_numbers() -> None:
    ...
    first = toolkit.run_strategy("sobol_control", 2_048, contract, market, 0.35)
    second = toolkit.run_strategy("sobol_control", 2_048, contract, market, 0.35)
...
def test_stratified_strategy_uses_randomized_replicate_interval() -> None:
    toolkit = VarianceReductionToolkit(seed_sequence=SeedSequence(77))
    ...
    assert "stratified" in result.model_used
