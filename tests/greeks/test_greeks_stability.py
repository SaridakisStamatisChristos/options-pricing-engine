...
def test_mc_greeks_accuracy_and_ci(golden_grid: list[BenchmarkScenario], paths: int) -> None:
    model = MonteCarloModel(
        paths=paths, antithetic=True, seed_sequence=None, use_control_variates=True
    )
    ...
    for scenario, seed in zip(golden_grid, scenario_seeds, strict=False):
        ...
        error_buckets["delta"].append(
            _relative_error(mc_result.delta or 0.0, bs_result.delta or 0.0)
        )
        error_buckets["gamma"].append(
            _relative_error(mc_result.gamma or 0.0, bs_result.gamma or 0.0)
        )
        error_buckets["theta"].append(
            _relative_error(mc_result.theta or 0.0, bs_result.theta or 0.0)
        )
