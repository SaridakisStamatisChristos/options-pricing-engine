...
def test_monotonicity_convexity_parity(golden_grid: list[BenchmarkScenario], paths: int) -> None:
    ...
    for scenario in golden_grid:
        ...
        price_up_sigma = model.calculate_price(
            contract, market, volatility + bump_sigma, seed_sequence=seed
        )
        ...
        price_down_tau = model.calculate_price(
            contract_down, market, volatility, seed_sequence=seed
        )
        ...
        price_down_k = model.calculate_price(
            contract_down_k, market, volatility, seed_sequence=seed
        )
    ...
    for (strike, tau, vol), market in market_lookup.items():
        ...
        if call_price is None or put_price is None:
            continue
        forward_anchor = market.spot_price * math.exp(
            -market.dividend_yield * tau
