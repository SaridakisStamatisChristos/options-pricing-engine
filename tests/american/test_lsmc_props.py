...
def test_strike_convexity() -> None:
    delta = 2.0
    lower = american_lsmc_price(
        **{**DEFAULT_ARGS, "strike": DEFAULT_ARGS["strike"] - delta, "seed": 19}
    )
    centre = american_lsmc_price(**{**DEFAULT_ARGS, "seed": 19})
    upper = american_lsmc_price(
        **{**DEFAULT_ARGS, "strike": DEFAULT_ARGS["strike"] + delta, "seed": 19}
    )
    lhs = centre.price
...
def test_non_dividend_american_call_uses_exact_theorem() -> None:
    ...
    assert "no_early_exercise_theorem" in set(deep_call.meta.get("policy_flags", []))
    assert deep_call.ci_half_width == 0.0
